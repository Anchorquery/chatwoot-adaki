...
class Platform::Credential < ApplicationRecord
  ...
  has_many :models, dependent: :destroy, class_name: '::Platform::CredentialModel', inverse_of: :credential

  encrypts :encrypted_payload if Chatwoot.encryption_configured?

  enum status: { active: 0, invalid_credential: 1, expired: 2, revoked: 3 }, _prefix: :status
  enum auth_type: {
    api_key: 'api_key',
    bearer: 'bearer',
    basic: 'basic',
